...
droptables = "DROP TABLE IF EXISTS lopp, tvl, prog;"

createlopp = """CREATE TABLE lopp
(
    id        int        NOT NULL,
...
createindices = """CREATE INDEX tvldatum_ix ON tvl (datum,bana,lopp);
...
try:
    ...
    conn = psycopg2.connect("host=127.0.0.1 port=5432 dbname=trav user=postgres password=ixman")
    cur = conn.cursor()
    cur.execute(droptables, createlopp)
    cur.execute(createtvl, createprog)
    cur.execute(createindices)
    cur.close()
    conn.commit()
